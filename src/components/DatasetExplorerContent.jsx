...
class ResultsComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.view) {
      ...
      this.setState({
        ...
        optionSelected: columnsData.columns.filter(column => column.show),
        pivotBy: (pivotBy || []),
        placeholder: `Displaying ${columnsData.shownColumnsCount} of ${columnsData.columns.length} Columns`
      })
    }
  }
  ...
  onColumnToggle(selectionObj, event) {
    ...
    if (event.action === "clear") {
      this.setState({
        columns: this.generateClearedColumns(this.state.columns),
        optionSelected: [],
        placeholder: `Displaying 0 of ${this.state.columns.length} Columns`
      })
      return
    }
    const objIndex = this.state.columns.findIndex(obj => obj.accessor === event.option.accessor);
    ...
    if (event.option.Header === "Select all") {
      ...
      this.setState(prevState => ({
        ...
        optionSelected: selected_columns,
        placeholder: `Displaying ${col_length} of ${this.state.columns.length} Columns`
      }))
    }
  }
        ...
        <div className='dataset-table-menubar'>
          <div>
            ...
            <Select
              ...
              defaultValue={options.filter(option => option.show)}
              isMulti={true}
        ...
        if (json.results) {
          ...
          Promise.all(filter_list.map(filters => {
            ...
            this.setState({
              ...
              series: listOfSeries,
              legendCategories: legendCategories.concat().sort(function (a, b) {
...
class DatasetExplorerContentComponent extends React.Component {
  ...
  render(): ?React.Element {
    ...
    if (this.props.visualization === true) {
      return (
        <div
          className={'dataset-explorer-content flex-content ' + (this.props.displayFilters ? 'width-77' : 'width-100 pad-r-30')}
