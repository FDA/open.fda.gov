  ...
  render() {
    ...
    return (
      <section className="doc-content">
        ...
        <APIUseSteps endpoint_name='Animal & Veterinary Adverse Events' endpoint_path='/apis/animalandveterinary/event/'/>
        ...
        <p>In the example below, we are searching the records in the animal & veterinary labeling endpoint for matches with <code>Veterinarian</code> in the <code>primary_reporter</code> field. We are requesting to see the first 5 records that match."</p>
        <APIQueryBreakdown endpoint_path='/animalandveterinary/event.json' query='primary_reporter:Veterinarian'/>

        <h3>Some key pointers</h3>
        <ul>
          <li>An openFDA API query always begins with the base endpoint, which in this case is: <code>https://api.fda.gov/animalandveterinary/event.json</code></li>
          <li>Searches have a special syntax: <code>search=field:term</code></li>
          <li>Unless otherwise specified, the API will return only one matching record for a search. You can specify the number of records to be returned by using the limit parameter. The maximum limit allowed is 100 for any single API call. If no limit is set, the API will return one matching record.</li>
        </ul>
